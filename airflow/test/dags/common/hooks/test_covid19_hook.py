...
@pytest.mark.parametrize("test_input,expected",
                         [
                             ([[], []], []),
                             ([list(filter(lambda x: x['Country'] == 'Portugal', sample_countries_data)),
                               sample_data_pt], [sample_data_pt]),
...
def test_get_data_valid_responses(mock, test_input, expected, dag):
    ...
    hook = Covid19Hook()
    data = list(hook.get_data(start_date='2020-07-12', end_date='2020-07-12', rate_limit=0))
    if len(test_input) > 0:
...
def test_execute_invalid_status_code(mock, dag):
    with pytest.raises(Exception) as excinfo:
        ...
        hook = Covid19Hook()
        list(hook.get_data(start_date='2020-07-12', end_date='2020-07-13', rate_limit=0))
...
@patch.object(HttpHook, 'run')
@patch('time.sleep')
def test_rate_limit(time_mock, http_mock, test_input, expected, dag):
    rate_limit = 5
    http_mock.side_effect = list(map(lambda x: MockResponse(x, 200), test_input))
    hook = Covid19Hook()
    data = list(hook.get_data(start_date='2020-07-12', end_date='2020-07-12', rate_limit=rate_limit))
    time_mock.assert_any_call(rate_limit)
    assert data == expected
